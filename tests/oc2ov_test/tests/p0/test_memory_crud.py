...
class TestMemoryUpdate(BaseOpenClawCLITest):
    ...
    def test_memory_update_verify(self):
        ...
        self.wait_for_sync()
        resp2 = self.send_and_log("我现在住在哪里？")
        self.assertAnyKeywordInResponse(resp2, [["西北"]], case_sensitive=False)
...
class TestMemoryDelete(BaseOpenClawCLITest):
    ...
    记忆删除验证测试
    测试目标：验证记忆删除功能是否正常
    测试场景：写入临时信息，验证存在后删除，再验证信息已被删除
    """

    def test_memory_delete_verify(self):
        """测试场景：信息删除与验证"""
        self.logger.info("[1/3] 写入待删除的测试信息")
        self.send_and_log("这是一条临时信息，我马上会删除它，我的临时密码是temp12345")
        self.wait_for_sync()
...
class TestMemoryUpdateOverwrite(BaseOpenClawCLITest):
    ...
    def test_memory_update_overwrite_group_a(self):
        ...
        message_initial = "我今年30岁"
        session_a = "update_overwrite_a"

        self.send_and_log(message_initial, session_id=session_a)
        self.wait_for_sync()

        self.logger.info("[2/4] 写入更新信息：我今年31岁，生日在8月")
        message_update = "我今年31岁，生日在8月"
        self.send_and_log(message_update, session_id=session_a)
        self.wait_for_sync()
    ...
    def test_memory_update_overwrite_group_b(self):
        ...
        message_initial = "我今年26岁"
        session_b = "update_overwrite_b"

        self.send_and_log(message_initial, session_id=session_b)
        self.wait_for_sync()

        self.logger.info("[2/4] 写入更新信息：我今年27岁，生日在11月")
        message_update = "我今年27岁，生日在11月"
        self.send_and_log(message_update, session_id=session_b)
        self.wait_for_sync()
    ...
    def test_memory_update_overwrite_group_c(self):
        ...
        message_initial = "我今年32岁"
        session_c = "update_overwrite_c"

        self.send_and_log(message_initial, session_id=session_c)
        self.wait_for_sync()

        self.logger.info("[2/4] 写入更新信息：我今年33岁，生日在5月")
        message_update = "我今年33岁，生日在5月"
        self.send_and_log(message_update, session_id=session_c)
        self.wait_for_sync()
